...
function addNewBookmark(bookmark){
    ...
    newBookmark.setAttribute("url", bookmark.maangURL);
    newBookmark.setAttribute("bookmark-id", bookmark.UniqueID);
...
function deleteItemFromStorage(idToRemove){
    chrome.storage.sync.get({bookmarks:[]}, (data) => {
        const currentBookmarks = data.bookmarks;
        const updatedBookmarks = currentBookmarks.filter((bookmark) => bookmark.UniqueID !== idToRemove);
